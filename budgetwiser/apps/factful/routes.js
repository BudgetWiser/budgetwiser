        ...
        if(_objList.length > 0){
            var articleList =
                '<div class="article-list-header">' +
                    '<h1 class="article-list-title">기사 리스트</h1>' +
                    '<button class="random-user">당신의 포켓몬 아이디: ' + req.user.username + '</button>' +
                '</div>' +
                '<link rel="stylesheet" type="text/css" href="/static/css/factful/layout.css">' +
...
function setup(app){
    ...
    // article add
    app.get('/factful/add/a0d98v7a', view.articleAdd);
